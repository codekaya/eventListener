...
async function main() {
  ...
  let proxyBlockNumber = 1;

  for (let x = 0; x < transferEvents.length; x++) {
    if (transferEvents[0] != proxyBlockNumber) {
      fs.writeFileSync(transferEvents[0].toString(), data, (err) => {
        if (err) {
          throw err;
        }
    ...
    proxyBlockNumber = transferEvents[0];
  }

  /*const isExist = [];

  const wallets = filtered.map((element, index, array) => {
    if (
      isExist.includes(element.topics[1]) &&
      isExist.includes(element.topics[2])
    ) {
      //element.transfers.push({otherAddress:})
